...
void DASSearch::statistics() const {
...
int DASSearch::step() {
	if(time_left()<0){
	//TODO
	std::cout<<"Ran out of time"<<endl;
	return FAILED;
	}
	//done below i think dmax=calc d bound
    pair<SearchNode, bool> n = fetch_next_node();
    if (!n.second) {
std::cout<<"derp calling unprune"<<endl;
	if(!recover_pruned_states())
            return FAILED;
         return step();
//return FAILED;
    }
...
pair<SearchNode, bool> DASSearch::fetch_next_node() {
...
void DASSearch::reward_progress() {
    // Boost the "preferred operator" open lists somewhat whenever
    // one of the heuristics finds a state with a new best h value.
    open_list->boost_preferred();
}

void DASSearch::dump_search_space() {
    search_space.dump();
}

void DASSearch::update_jump_statistic(const SearchNode &node) {
    if (f_evaluator) {
...
void DASSearch::print_heuristic_values(const vector<int> &values) const {
